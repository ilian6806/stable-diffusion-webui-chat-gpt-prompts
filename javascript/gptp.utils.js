...
gptp.utils = {
    ...
    clickToggleMenu: function openToggleMenu(element) {
        if (! element) {
            gptp.logging.warn('Toggle button not found');
            return;
        }
        element.click();
        return element;
        ...
        try {
            ...
            if (value) {
                ...
                setTimeout(() => {
                    ...
                    gptp.utils.triggerMouseEvent(input, 'focus');

                    setTimeout(() => {
                        let items = Array.from(select.querySelectorAll('ul li'));
                        items.forEach(li => {
                            if (li.lastChild.wholeText.trim() === value) {
                                gptp.utils.triggerMouseEvent(li, 'mousedown');
                                return false;
                            }
                        });
                        gptp.utils.triggerMouseEvent(input, 'blur');
                        selectingQueue -= 1;
                    }, 100);
                }, selectingQueue * 200)
            }
            ...
            setTimeout(() => {
                gptp.utils.onContentChange(select, function (el) {
            ...
            console.error('[gptp]: Error:', error);
        }
    },
